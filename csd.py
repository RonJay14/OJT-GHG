...
def external_dashboard():
    ...
    try:
        ...
        # Count records
        cursor.execute("SELECT COUNT(*) AS total_records FROM tblflight WHERE campus = %s", (campus,))
        total_flight_records = cursor.fetchone()['total_records']

        cursor.execute("SELECT COUNT(*) AS total_records FROM tblaccommodation WHERE campus = %s", (campus,))
