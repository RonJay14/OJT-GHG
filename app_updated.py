    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user and password == user['password']:  # Correct password
                ...
                session['logo'] = f"images/campuses/{campus_logo_map.get(user['campus'], 'default.png')}"
...
def emu_dashboard():
    ...
    try:
        ...
        queries = [
            ...
            ("SELECT MONTHNAME(Date) AS month, SUM(FactorKGCO2e) AS total_emission FROM tblwater WHERE YEAR(Date) = %s AND campus = %s GROUP BY MONTHNAME(Date) ORDER BY FIELD(MONTHNAME(Date), 'January', 'February', 'March', 'April', 'May', 'June', 'August', 'September', 'October', 'November', 'December')", water_data, "water"),
        ]
...
def electricity_consumption():
    ...
    if request.method == 'POST':
        ...
        tree_offset = request.form.get('tree_offset')
        ...
        try:
            ...
            sql = """INSERT INTO electricity_consumption 
                      (campus, category, month, quarter, year, prev_reading, current_reading, multiplier, total_amount, consumption, price_per_kwh, kg_co2_per_kwh, t_co2_per_kwh, tree_offset)
                      VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            cursor.execute(sql, (campus, category, month, quarter, year, prev_reading, current_reading, multiplier, total_amount, consumption, price_per_kwh, kg_co2_per_kwh, t_co2_per_kwh, tree_offset))
            
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Electricity Consumption Report for {campus} - {month} {year} ({category})"
...
def water_consumption():
    ...
    if request.method == 'POST':
        try:
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Water Consumption Report for {campus} - {date}"
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
...
def treated_water():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Treated Water Report for {campus} - {month}"
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
...
def emu_fuel():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Fuel Emissions Report for {campus} - {date}"
            
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "Fuel Emissions Report"))
...
def delete_fuel_record(id):
    if 'campus' not in session or 'username' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    try:
        ...
        cursor.execute("""INSERT INTO activity_log 
                         (username, campus, action, report_name)
                         VALUES (%s, %s, %s, %s)""",
                      (session['username'], session['campus'], "deleted", f"Fuel Record ID: {id}"))
...
def waste_segregation():
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                ghg_emission_kg = float(request.form.get('ghgEmissionKg', 0))
                ghg_emission_t = float(request.form.get('ghgEmissionT', 0))
            ...
            try:
                ...
                sql = """INSERT INTO tblsolidwastesegregated 
                        (Campus, Year, Month, Quarter, MainCategory, SubCategory, 
                         QuantityInKG, GHGEmissionKGCO2e, GHGEmissionTCO2e)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                
                # Print SQL and parameters for debugging
                params = (campus, year, month, quarter, main_category, sub_category,
                         quantity, ghg_emission_kg, ghg_emission_t)
                print("SQL Query:", sql)
                ...
                # Log the activity
                username = session.get('username', 'Unknown')
                action = f"Generated Waste Segregation Report for {campus} - {month} {year}"
                cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                ...
                return jsonify({
                    'success': True,
                    'message': 'Waste segregation record added successfully'
                })
    ...
    try:
        ...
        return render_template('waste_segregation.html', reports=[])
...
def add_waste_unsegregated():
    try:
        ...
        log_sql = """
            ...
            VALUES (%s, %s, %s, %s)
        """
        action = f"Generated waste unsegregation report for {campus} - {month} {year}"
...
def csdanalytics():
    ...
    try:
        ...
        queries = [
            ...
            ("SELECT Month, SUM(GHGEmissionKGCO2e) AS total_emission FROM tblsolidwastesegregated WHERE Year = %s {campus_filter} GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", waste_segregated_data, "waste_segregated"),
            ("SELECT Month, SUM(GHGEmissionKGCO2e) AS total_emission FROM tblsolidwasteunsegregated WHERE Year = %s {campus_filter} GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", waste_unsegregated_data, "waste_unsegregated"),
            ("SELECT MONTHNAME(Date) AS month, SUM(FactorKGCO2e) AS total_emission FROM tblwater WHERE YEAR(Date) = %s {campus_filter} GROUP BY MONTHNAME(Date) ORDER BY FIELD(MONTHNAME(Date), 'January', 'February', 'March', 'April', 'May', 'June', 'August', 'September', 'October', 'November', 'December')", water_data, "water"),
        ]
        ...
        additional_queries = [
            ("SELECT Month AS month, SUM(FactorKGCO2e) AS total_emission FROM tbltreatedwater WHERE YEAR(CURDATE()) = %s {campus_filter} GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", treated_water_data, "treated_water"),
            ("SELECT Month AS month, SUM(GHGEmissionKGCO2e) AS total_emission FROM tblfoodwaste WHERE YearTransaction = %s {campus_filter} GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", food_waste_data, "food_waste"),
            ("SELECT Month AS month, SUM(GHGEmissionKGCO2e) AS total_emission FROM tbllpg WHERE YearTransact = %s {campus_filter} GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'August', 'September', 'October', 'November', 'December')", lpg_data, "lpg"),
        ]
...
@app.route('/accommodation_emissions_report/data')
def get_accommodation_emissions_data():
    if 'loggedIn' not in session:
        return jsonify({"error": "Not logged in"}), 401
    ...
    try:
        ...
        if request.args.get('office'):
            query += " AND Office = %s"
            params.append(request.args.get('office'))
        ...
        for record in data:
            if record.get('TravelDateFrom'):
                record['TravelDateFrom'] = record['TravelDateFrom'].strftime('%Y-%m-%d')
            if record.get('TravelDateTo'):
                record['TravelDateTo'] = record['TravelDateTo'].strftime('%Y-%m-%d')
...
@app.route('/report/food_consumption', methods=['GET', 'POST'])
def food_consumption_report():
    if request.method == 'POST':
        try:
            ...
            action = f"Generated Food Consumption Report for {campus} - {month} {year}"
            
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "Food Consumption Report"))
            ...
            return redirect(url_for('food_consumption_report'))
...
@app.route('/report/flight_emissions', methods=['GET', 'POST'])
def flight_emissions_report():
    if request.method == 'POST':
        try:
            ...
            ghg_emission_kg = request.form.get('ghgEmissionKGC02e')
            ghg_emission_t = request.form.get('ghgEmissionTC02e')

            # Connect to database
            conn = get_db_connection()
            cursor = conn.cursor()

            # Insert data into the database - using %s placeholders for MySQL
            cursor.execute("""
                INSERT INTO tblflight (
                    Campus, Office, Year, TravellerName, TravelPurpose, TravelDate,
            ...
            """, (campus, office, year, traveller_name, travel_purpose, travel_date,
                  domestic_international, origin, destination, travel_class,
                  oneway_roundtrip, ghg_emission_kg, ghg_emission_t))
            
            # Log the activity
            username = session.get('username', 'Unknown')  # Get username from session
            action = f"Generated Flight Emissions Report for {campus} - {travel_date}"
            
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "Flight Emissions Report"))

            conn.commit()
            conn.close()

            flash('Flight data added successfully!', 'success')
            return redirect(url_for('flight_emissions_report'))

        except Exception as e:
            flash(f'Error adding flight data: {str(e)}', 'error')
            return redirect(url_for('flight_emissions_report'))
    ...
    data, total_pages = fetch_flight_data(page, per_page, campus, office, year)
...
@app.route('/flight_emissions_report/all')
def flight_emissions_report_all():
    # Get filter values from request args
    campus = request.args.get("campus", None)
    office = request.args.get("office", None)
    year = request.args.get("year", None)
    ...
    if campus:
        where_conditions.append("Campus = %s")
        params.append(campus)
    if office:
        where_conditions.append("Office = %s")
        params.append(office)
    if year:
    ...
    query = f"""
        SELECT Campus, Office, Year, TravellerName, TravelPurpose, TravelDate,
               Origin, Destination, Class, OnewayRoundTrip, 
               GHGEmissionKGC02e, GHGEmissionTC02e
...
@app.route('/report/accommodation_emissions', methods=['GET', 'POST'])
def accommodation_emissions_report():
    if request.method == 'POST':
        ...
        try:
            ...
            # Insert data into the database including the Factor column
            sql = """INSERT INTO tblaccommodation (Campus, Office, YearTransact, TravellerName, TravelPurpose, 
                     TravelDateFrom, TravelDateTo, Country, TravelType, NumOccupiedRoom, NumNightPerRoom,
                     Factor, GHGEmissionKGC02e, GHGEmissionTC02e)
                     VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            ...
            # Log the activity
            username = session.get('username', 'Unknown')  # Get username from session
            action = f"Generated Accommodation Emissions Report for {campus} - {travel_date_from}"
            
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "Accommodation Emissions Report"))
        ...
        return redirect(url_for('accommodation_emissions_report'))
...
@app.route('/sdo_fuel_emissions_report', methods=['GET', 'POST'])
def sdo_fuel_emissions_report():
    ...
    campus_mapping = {
        "Alangilan": ["Lobo", "Mabini", "Balayan", "Pablo Borbon"],
        "Pablo Borbon": ["Lemery", "San Juan", "Rosario", "Central", "Pablo Borbon"]
    }
    ...
    if request.method == 'POST':
        # Handle POST request - Add your logic here
        return jsonify({'success': True, 'message': 'POST request handled'})
    ...
    cursor.execute("""INSERT INTO activity_log 
                      (username, campus, action, report_name)
                      VALUES (%s, %s, %s, %s)""",
                   (username, user_campus, "viewed", "Fuel Emissions Report"))
    conn.commit()
    ...
    # Count total records
    count_query = f"SELECT COUNT(*) as total FROM fuel_emissions {where_clause}"
    ...
    return render_template(
        ...
        current_page=current_page,
        total_pages=total_pages,
        campus_filter=related_campuses,
        year_filter=year_filter
    )

@app.route('/edit_fuel_record/<int:id>', methods=['GET', 'POST'])
def edit_fuel_record(id):
    ...
    if request.method == 'GET':
        # Handle GET request - fetch record
        try:
            cursor.execute("SELECT * FROM fuel_emissions WHERE id = %s", (id,))
        ...
        try:
            data = {
                ...
                'vehicle_equipment': request.form['vehicle_equipment'],
            ...
            cursor.execute("""
                ...
                WHERE id=%s
            """, (data['date'], data['driver'], data['type'], 
                  data['vehicle_equipment'], data['plate_no'], 
                  data['category'], data['fuel_type'],
                  data['quantity_liters'], data['total_amount'], id))
            ...
            cursor.execute("""
                ...
                (username, campus, action, report_name)
                VALUES (%s, %s, %s, %s)
            """, (session['username'], session['campus'], 
                  "edited", f"Fuel Record ID: {id}"))
...
def export_fuel_report_pdf():
    ...
    # Build base query
    query = """
        SELECT campus, date, driver, type, vehicle_equipment, plate_no, 
               category, fuel_type, quantity_liters, total_amount,
               co2_emission, nh4_emission, n2o_emission, 
               total_emission, total_emission_t
        FROM fuel_emissions
        WHERE campus = %s
    """
...
def manageacc_sdo():
    ...
    # Fetch accounts and activity logs for the user's campus
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        ...
        # Fetch activity logs for the current campus
        cursor.execute("""
            SELECT username, campus, action, report_name, timestamp 
            FROM activity_log 
            WHERE campus = %s
    ...
    return render_template('manageacc_sdo.html', accounts=accounts, activity_logs=activity_logs)
...
def accommodation():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Accommodation Report for {campus} - {year}"
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "Accommodation Report"))
            conn.commit()
...
@app.route('/accommodation_emissions_report/all')
def get_all_accommodation_emissions():
    if 'loggedIn' not in session:
        return jsonify({"error": "Not logged in"}), 401
    ...
    try:
        ...
        if request.args.get('campus'):
            query += " WHERE Campus = %s"
            params.append(request.args.get('campus'))
            
        if request.args.get('office'):
            query += " AND Office = %s" if params else " WHERE Office = %s"
            params.append(request.args.get('office'))
            
        if request.args.get('year'):
            query += " AND YearTransact = %s" if params else " WHERE YearTransact = %s"
            params.append(request.args.get('year'))
...
def food_consumption():
    ...
    if request.method == 'POST':
        try:
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated Food Consumption Report for {campus} - {month} {year} ({food_type}, {servings} servings)"
...
@app.route('/food_consumption_report/all', methods=['GET'])
def food_consumption_report_all():
    if 'loggedIn' not in session:
    ...
    try:
        ...
        # Base query
        sql = "SELECT * FROM tblfoodwaste WHERE 1=1"
        params = []
...
def lpg_consumption():
    ...
    if request.method == 'POST':
        try:
            ...
            # Log the activity
            username = session.get('username', 'Unknown')
            action = f"Generated LPG Consumption Report for {campus} - {month} {year} ({concessionaires}, {qty} tanks)"
            
            cursor.execute("""INSERT INTO activity_log 
                          (username, campus, action, report_name)
                          VALUES (%s, %s, %s, %s)""",
                       (username, campus, action, "LPG Consumption Report"))
